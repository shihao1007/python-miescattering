"""
4.1 Edition of Mie-Scattering Simulation
...
Using the same k vectors sampled by monte carlo sampling so the
...
class mieScattering:
    ...
    def __init__(self, k, k_j, n, res, a, numSample, NA_in, NA_out, option = 'Horizontal'):
        ...
        if self.option == 'Horizontal':
            ...
            self.z = np.zeros((self.simRes, self.simRes,)) + 10
...
def getTotalField(k, k_j, n, res, a, numSample, NA_in, NA_out, option):
    #root function to get the final field by call other children functions
    
    #initialize a mie scattering object
    MSI = mieScattering(k, k_j, n, res, a, numSample, NA_in, NA_out, option)  
...
#
kObj = mieScattering(k, [], n0, res, a, numSample, NA_in, NA_out, 'Horizontal')
k_j = kObj.sampled_kvectors_spherical_coordinates(kObj.NA_in, kObj.NA_out, kObj.numSample, kObj.k)
#
#for i, n in enumerate(N):
#    Ef_v[:,:,i], Et_v[:,:,i], Dt_v[:,:,i], Df_v[:,:,i] = getTotalField(k, k_j, n, res, a, numSample, NA_in, NA_out, 'Vertical')
...
#ani.save(parentDir + '\\Et_v.mp4',writer=writer)

Ef = kObj.calFocusedField(kObj.simRes, kObj.magk, kObj.rMag)
#Ef_v, Et_v, Dt_v, Df_v = getTotalField(k, k_j, n0, res, a, numSample, NA_in, NA_out, 'Vertical')
Ef_h, Et_h, Dt_h, Df_h = getTotalField(k, k_j, n0, res, a, numSample, NA_in, NA_out, 'Horizontal')

#plt.figure()
#plt.imshow(np.abs(Et_v))
#plt.set_cmap('viridis')
#plt.axis('off')
#plt.colorbar()

plt.figure()
plt.imshow(np.abs(Et_h))
plt.set_cmap('viridis')
plt.axis('off')
plt.colorbar()
